...
/// Симметричное E2E-шифрование переписки между двумя узлами.
///
/// Алгоритм:
///   • Ключ = SHA-256(alias1 + ":" + alias2), где узлы отсортированы
///     лексикографически. Одинаков у обоих собеседников.
///   • AES-256-CBC + PKCS7 (реализован вручную).
///   • IV = первые 16 байт SHA-256 от `iv:$packetId`. Уникален
///     для каждого пакета.
///   • Каждое сообщение подписано HMAC-SHA256(ключ, packetId||ct).
///     Проверка HMAC происходит ДО расшифровки — это защищает от
///     padding-oracle атаки.
///   • В выходной строке формат: `enc:<base64>:<sig>`.
///
/// Что видит посредник (relay):
///   • Только base64-блоб. У него нет ключа — он не может
///     ни прочитать, ни подделать сообщение.
///   • Посредник хранит блоб в MessageStore и пересылает дальше
///     по цепочке, пока TTL > 0. Сообщение дойдёт до адресата
///     даже через 5–8 узлов.
///
/// Что может прочитать отправитель:
///   • Подписывает пакет HMAC-SHA256 в `IdentityService.sign`.
///   • Шифрует текст для адресата. Сам отправитель может
///     расшифровать своим же ключом (sharedKey = sorted aliases).
///
/// Что может прочитать адресат:
///   • Получает зашифрованный блоб, расшифровывает тем же ключом.
///   • Проверяет HMAC — если подпись неверна, расшифровка
///     отклоняется (вернётся null).
///
/// Чаты в городе (room) — публичные, без шифрования.
/// Broadcast-объявления — тоже публичные (флаг `clear: true`).
class SecureChat {
